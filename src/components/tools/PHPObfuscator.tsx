...
const PHPObfuscator: React.FC = () => {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [copied, setCopied] = useState(false);

  // Simple PHP obfuscation function
  const obfuscateCode = (code: string) => {
    let obfuscated = code;
    
    // Replace variable names with random strings
    const variables = code.match(/\$[a-zA-Z_][a-zA-Z0-9_]*/g) || [];
    ...
    Object.entries(varMap).forEach(([original, obfuscated]) => {
      const regex = new RegExp(`\\${original.replace('$', '\\$')}\\b`, 'g');
      obfuscated = obfuscated.replace(regex, obfuscated);
    });
    
    // Replace function names (except built-in ones)
    ...
    functions.forEach((func) => {
      ...
      funcMap[funcName] = `_${Math.random().toString(36).substr(2, 8)}`;
    });
    
    Object.entries(funcMap).forEach(([original, obfuscated]) => {
...
?>`;
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">PHP Code Obfuscator</h1>
        <p className="text-gray-400">Obfuscate PHP code with real-time preview</p>
      </div>
